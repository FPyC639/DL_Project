...
def make_cpp_membound_code_prefix(limit_mb):
...
def containerized_run(item, limit_mb=4 * 1024):
    ...
    membound_code = make_cpp_membound_code_prefix(limit_mb) + code
...
def extract_code(response: str) -> str:
    def sanitize_codeblock(code: str) -> str:
        if "input" not in code:
            return code.strip()
        # Only remove the `"int main("` when `input` is present because
...
def main():
    ...
    raw_data = load_dataset(args.response_paths, split="train")
    if args.shuffle:
...
if __name__ == "__main__":
    ...
    # response_paths = an778/self-oss-instruct-sc2-responses
    main()