...
def exec_test(server, code, test, timeout=30, timeout_on_client=False, stdin="") -> Tuple[bool, str]:
    """
    Executes a test against a C++ code snippet.
    Returns True if the test passes, False otherwise.
    Also returns the output of the code (stderr if it fails, stdout if it passes).

    Set `test` to an empty string if you want to execute the code without any tests
    ...
    data = json.dumps(
        {"language": "cpp", "code": code_with_tests, "timeout": timeout, "stdin": stdin})
    try:
        r = requests.post(
            server + "/cpp_exec",
            data=data,
